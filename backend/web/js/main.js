...
base_url=base_url[0];
base_url=base_url+'web/';//general/ajaxactiongetsectors'
...
/////
  ...
  function myfunction(all_shades){

        $.ajax({
           url: base_url + 'order/totalshades',
    ...
    var lastWord = document.getElementById("inputfield").value.split("\n");//.split("/n").splice(-1)[0];
    var all_shades = all_shades; //from php
    ...
    allWords.forEach(function(lastWord)
    {
          ...
          // else 
          console.log('all data '+ all_shades[1]);

           if (lastWord != "")
            {
              console.log('r');
              lastWord=all_shades[lastWord-1];}
          if (lastWord == "")
          {console.log("hhhhhhh");}
          // else
           else
